...
const fetchWard = async (district, province) => {
	const res = await fetch(
		`/address/wards/?districtID=${district}&provinceID=${province}`
	);
	const data = await res.json();
	return data;
};
...
const fetchStreet = async (district, province) => {
	const res = await fetch(
		`/address/streets/?districtID=${district}&provinceID=${province}`
	);
	const data = await res.json();
	return data;
};
...
const renderCategory = async (data) => {
	for (let item of data) {
		category.options[category.options.length] = new Option(
			item.name,
			item.categoryid
		);
	}
...
const renderAddress = async (data) => {
	...
	ward.onchange = async () => {
		...
		for (let s of data) {
			street.options[street.options.length] = new Option(
				s.namestreet,
				s.streetid
			);
		}
